...
def _build_summary_rows(stats: dict, items: list[dict]) -> list[tuple[str, int | float]]:
    total = len(items)
...
def export_items_to_excel(items: list[dict], stats: dict, output_dir: Path, keyword: str) -> Path:
    ...
    ws = wb.active
    ws.title = "items_view"
    _write_header(ws, VIEW_COLUMNS)
    _write_rows(ws, VIEW_COLUMNS, items)
    _color_status(ws, VIEW_COLUMNS)
    ws.freeze_panes = "A2"
    _auto_fit(ws)

    ws_raw = wb.create_sheet("items_raw")
    _write_header(ws_raw, RAW_COLUMNS)
    _write_rows(ws_raw, RAW_COLUMNS, items)
    ...
    for idx, (k, v) in enumerate(_build_summary_rows(stats, items), start=2):
...
def main() -> None:
    ...
    output_path = export_items_to_excel(items=items, stats=data.get("stats") or {}, output_dir=Path(args.output_dir), keyword=args.keyword)
