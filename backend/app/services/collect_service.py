...
class CollectService:
    ...
    # ── HTML meta 提取（正则，无需外部 HTML 解析库）──
    @staticmethod
    def _extract_meta(html: str, prop: str) -> str:
        """提取 <meta property/name=prop content=...> 的值"""
        patterns = [
    ...
    async def fetch_url_meta(url: str) -> Tuple[bool, Dict[str, str]]:
        """
        尝试抓取页面并提取 og/meta 标签，
        返回 (success, {title, description, author, site_name})
        """
        try:
            ...
            logger.warning(f"fetch_url_meta failed for {url!r}: {e}")
            return False, {}
    ...
    def get_list(
        ...
        q = db.query(ContentAsset).filter(ContentAsset.owner_id == user_id)
        if platform and platform != "all":
            q = q.filter(ContentAsset.platform == platform)
        if category and category != "all":
            q = q.filter(ContentAsset.category == category)
        if is_viral is not None:
            q = q.filter(ContentAsset.is_viral == is_viral)
        if search:
            q = q.filter(
                or_(
        ...
        return q.order_by(desc(ContentAsset.created_at)).offset(skip).limit(limit).all()
    ...
    async def analyze_with_ai(
        ...
        """
        调用 LLM 分析内容：生成标签、分类、爆款评分、改写建议，
        以及传播潜力、用户画像、情绪调性、内容结构、可复用元素、风险预警等扩展维度。
        失败时降级为规则分类。
        """
        ...
        try:
            ...
            if m:
                ...
                return {
                    ...
                    "viral_reasons":      [str(r) for r in data.get("viral_reasons", [])][:5],
                    "key_selling_points": [str(p) for p in data.get("key_selling_points", [])][:5],
            ...
            logger.error(f"AI analyze failed for content_id={content.id}: {e}")
