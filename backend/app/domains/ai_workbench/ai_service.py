...
class AIService:
    ...
    async def _call_ollama(self, prompt: str, system_prompt: str = "") -> str:
        try:
            ...
            raise Exception(f"Failed to call Ollama: {exc}")
    ...
    async def _post_ark_responses(
        ...
        try:
            ...
            raise Exception(f"Failed to call Ark API: {exc}")
...
- 禁止使用【一定放款】【100%下款】等违规词语"""
    ...
    async def rewrite_douyin(
        ...
        system_prompt = "你是抖音短视频剧本专家，擅长金融获客类内容。用中文回复。"
        ...
        if insight_ctx and insight_ctx.get("reference_count", 0) > 0:
            ...
            if insight_ctx.get("hook_examples"):
                parts.append("《开头钉子类型》: " + "、".join(insight_ctx["hook_examples"][:3]))
            if insight_ctx.get("pain_point_examples"):
                parts.append("《抖音同类目标痛点》: " + "、".join(insight_ctx["pain_point_examples"][:5]))
            if insight_ctx.get("structure_examples"):
...
- 禁用违规承诺词语"""
    ...
    async def rewrite_zhihu(
        ...
        system_prompt = "你是知乎专业回答作者，擅长金融信贷误区避坑、个人信贷策略类回答。用中文回复。"
...
- 禁用担保、承诺类违规词语"""
    ...
    async def rewrite_multi_version(
        ...
        Returns a list of {style, style_label, content, predicted_engagement, predicted_conversion}.
        """
        if styles is None:
