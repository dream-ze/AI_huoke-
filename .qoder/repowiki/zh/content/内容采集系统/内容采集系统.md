...
- [desktop/src/pages/collect-center/CollectCenterPage.tsx](file://desktop/src/pages/collect-center/CollectCenterPage.tsx)
...
- [docs/operations/浏览器插件联调与回归清单_2026-03-23.md](file://docs/operations/浏览器插件联调与回归清单_2026-03-23.md)
</cite>

## 更新摘要
**所做更改**
- 新增浏览器客户端从外部HTTP调用改为内部直接调用的技术实现
- 新增智能路由逻辑和单链接详情采集功能
- 更新采集系统架构图，展示新的内部调用模式
- 新增前端API客户端适配不同运行环境的配置机制
- 新增桌面端Electron应用的API基础URL动态配置功能
...
**更新** 本版本重点介绍了全新的MVP inbox系统架构，该系统完全替代了原有的collector模块，提供了更加强大和智能化的内容管理功能。同时，浏览器客户端已从外部HTTP调用改为内部直接调用模式，新增智能路由逻辑和单链接详情采集功能，显著提升了采集效率和系统稳定性。
...
- **BrowserCollectorClient**：浏览器采集客户端，支持内部直接调用和HTTP外部调用两种模式
- **CollectService**：采集服务，提供平台识别、URL元数据提取、AI分析等功能

**章节来源**
- [backend/app/services/mvp_inbox_service.py:7-136](file://backend/app/services/mvp_inbox_service.py#L7-L136)
- [backend/app/services/mvp_material_service.py:7-158](file://backend/app/services/mvp_material_service.py#L7-L158)
- [backend/app/services/mvp_tag_service.py:38-144](file://backend/app/services/mvp_tag_service.py#L38-L144)
- [backend/app/collector/services/browser_client.py:15-132](file://backend/app/collector/services/browser_client.py#L15-L132)
- [backend/app/collector/services/collect_service.py:74-285](file://backend/app/collector/services/collect_service.py#L74-L285)
...
**更新** 浏览器采集客户端已从外部HTTP调用改为内部直接调用模式：

- **智能路由逻辑**：默认使用内部直接调用模式，仅在检测到非本地地址时才回退到HTTP调用
- **内部调用优势**：避免HTTP开销和Connection refused问题，提升采集效率
- **单链接详情采集**：新增collect_single_link方法，专门处理单个链接的详情采集
- **平台支持**：目前支持小红书平台的内部详情采集

```mermaid
classDiagram
class BrowserCollectorClient {
+run_collect(platform, keyword, max_items)
+_run_collect_internal(platform, keyword, max_items)
+collect_keyword(platform, keyword, max_items)
+collect_single_link(url)
+_collect_single_link_internal(platform, url)
+_is_local_url() bool
-_use_http bool
}
...
BrowserCollectorClient --> XiaohongshuCollector
```

**图表来源**
- [backend/app/collector/services/browser_client.py:15-132](file://backend/app/collector/services/browser_client.py#L15-L132)
- [backend/app/collector/parsers/xiaohongshu.py:18-144](file://backend/app/collector/parsers/xiaohongshu.py#L18-L144)

**章节来源**
- [backend/app/collector/services/browser_client.py:15-132](file://backend/app/collector/services/browser_client.py#L15-L132)

### 采集服务（CollectService）
**更新** 采集服务提供完整的平台识别和元数据提取功能：

- **平台识别**：基于正则表达式的智能平台识别，支持小红书、抖音、知乎等多个平台
- **URL元数据提取**：异步提取页面标题、描述、作者、站点名称等元数据
- **AI分析**：基于内容的自动分类和爆款分析
- **智能路由**：配合BrowserCollectorClient实现智能采集路由
...
### 前端API客户端适配
**更新** 前端API客户端已适配不同运行环境的配置机制：

- **运行时配置**：支持Electron桌面应用的动态API基础URL配置
- **环境检测**：自动检测运行环境（开发/生产/桌面应用）
- **配置持久化**：localStorage存储API基础URL和认证信息
- **请求拦截**：统一的请求拦截器处理认证和错误

**章节来源**
- [desktop/src/lib/api.ts:1-800](file://desktop/src/lib/api.ts#L1-L800)
- [desktop/src/App.tsx:39-76](file://desktop/src/App.tsx#L39-L76)

### 桌面端Electron应用集成
**更新** 桌面端Electron应用提供完整的API基础URL动态配置功能：

- **运行时端口配置**：支持通过applyRuntimeApiBaseUrl设置API端口
- **健康检查**：集成后端健康检查，确保服务可用性
- **本地代理**：桌面应用作为本地代理，避免跨域问题
- **配置持久化**：localStorage存储用户配置

**章节来源**
- [desktop/electron/main.cjs:106-146](file://desktop/electron/main.cjs#L106-L146)
- [desktop/src/App.tsx:42-47](file://desktop/src/App.tsx#L42-L47)
...
BROWSER_CLIENT["BrowserCollectorClient"] --> XHS_COLLECTOR["XiaohongshuCollector"]
COLLECT_SERVICE["CollectService"] --> BROWSER_CLIENT
FRONTEND_API["前端API客户端"] --> MVP_API
ELECTRON_APP["Electron桌面应用"] --> FRONTEND_API
```
...
- [backend/app/collector/services/browser_client.py:22-26](file://backend/app/collector/services/browser_client.py#L22-L26)
...
**更新** 特别是在浏览器客户端方面，从外部HTTP调用改为内部直接调用的改进，显著提升了系统的稳定性和性能。智能路由逻辑确保了在不同环境下都能选择最优的调用方式，而单链接详情采集功能则为特定场景提供了更精确的采集能力。前端API客户端的运行时配置机制和桌面端Electron应用的集成，进一步增强了系统的易用性和可靠性。
...
- **知识库配置**
  ...
  - library_type：知识库类型
- **浏览器客户端配置**
  - BROWSER_COLLECTOR_BASE_URL：采集器基础URL
  - BROWSER_COLLECTOR_TIMEOUT_SECONDS：超时时间（秒）
  - use_http：强制使用HTTP调用模式
...
- **采集结果**：包含平台、标题、内容、作者、提取状态、成本时间等字段
...
- **联调准备**：确保后端已启动并可访问/api/v1/ai/plugin/collect
- **安装测试**：支持Chrome和Edge浏览器的插件安装与测试
- **数据一致性**：验证plugin_collections、content_assets、insight_content_items的数据一致性
- **异常处理**：测试网络断开、重复采集、401/429/5xx等异常情况
- **性能验证**：验证内部调用模式相比HTTP调用的性能提升

**章节来源**
- [docs/operations/浏览器插件联调与回归清单_2026-03-23.md:1-56](file://docs/operations/浏览器插件联调与回归清单_2026-03-23.md#L1-L56)