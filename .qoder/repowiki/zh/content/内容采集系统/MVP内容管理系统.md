...
- [backend/app/api/endpoints/mvp_inbox_routes.py](file://backend/app/api/endpoints/mvp_inbox_routes.py)
- [backend/app/api/endpoints/mvp_material_routes.py](file://backend/app/api/endpoints/mvp_material_routes.py)
- [backend/app/api/endpoints/mvp_knowledge_routes.py](file://backend/app/api/endpoints/mvp_knowledge_routes.py)
- [backend/app/api/endpoints/mvp_generation_routes.py](file://backend/app/api/endpoints/mvp_generation_routes.py)
- [backend/app/api/endpoints/mvp_compliance_routes.py](file://backend/app/api/endpoints/mvp_compliance_routes.py)
- [backend/app/api/endpoints/mvp_tag_routes.py](file://backend/app/api/endpoints/mvp_tag_routes.py)
- [backend/app/api/endpoints/mvp_stats_routes.py](file://backend/app/api/endpoints/mvp_stats_routes.py)
- [backend/app/api/endpoints/mvp_feedback_routes.py](file://backend/app/api/endpoints/mvp_feedback_routes.py)
- [backend/app/api/endpoints/mvp_model_routes.py](file://backend/app/api/endpoints/mvp_model_routes.py)
- [backend/app/schemas/mvp_schemas.py](file://backend/app/schemas/mvp_schemas.py)
- [backend/app/models/mvp.py](file://backend/app/models/mvp.py)
...
## 更新摘要
**所做更改**
- 从单一mvp_routes.py重构为完整的MVP生态系统架构
- 新增收件箱管理、素材库、知识库、生成工作流、合规审核、标签管理、统计仪表盘、反馈闭环、模型管理等专门路由模块
- 实现四层处理管道系统：清洗→抽取→筛选→入库
- 扩展API端点至20个专门路由文件，支持完整的采集入库流水线
- 增强向量检索和知识图谱功能
- 新增模型管理和性能基准测试功能
...
MVP内容管理系统是一个基于FastAPI构建的智能内容创作平台，专注于金融行业的获客内容生成和管理。该系统经过重大重构，从单一mvp_routes.py演进为完整的MVP生态系统，集成了AI大模型技术，提供从内容采集、素材管理、知识库构建到AI生成的一站式解决方案。
...
- **知识图谱**：基于向量相似度和元数据的关系发现
- **反馈闭环**：生成结果的质量追踪和持续优化
- **模型管理**：支持多种AI模型的切换和性能对比
...
**架构图来源**
- [backend/app/api/router.py:21-46](file://backend/app/api/router.py#L21-L46)
- [backend/app/api/endpoints/mvp_routes.py:6-30](file://backend/app/api/endpoints/mvp_routes.py#L6-L30)
...
RouterManager --> FastAPIApp : registers
```
...
系统采用模块化路由设计，将MVP功能拆分为20个专门的路由模块：

```mermaid
graph LR
subgraph "MVP路由模块"
MVP[mvp_routes.py - 汇总入口]
Inbox[mvp_inbox_routes.py]
Material[mvp_material_routes.py]
Knowledge[mvp_knowledge_routes.py]
Generation[mvp_generation_routes.py]
Compliance[mvp_compliance_routes.py]
Tag[mvp_tag_routes.py]
Stats[mvp_stats_routes.py]
Feedback[mvp_feedback_routes.py]
Model[mvp_model_routes.py]
end
subgraph "其他路由模块"
Auth[auth_routes.py]
Content[content_routes.py]
Customer[customer_routes.py]
...
- **可扩展性**：新增功能只需添加新的路由模块和服务类

**章节来源**
- [backend/app/api/endpoints/mvp_routes.py:1-30](file://backend/app/api/endpoints/mvp_routes.py#L1-L30)
- [backend/app/api/router.py:1-46](file://backend/app/api/router.py#L1-L46)
...
**ER图来源**
- [backend/app/models/mvp.py:33-200](file://backend/app/models/mvp.py#L33-L200)

**章节来源**
- [backend/app/models/mvp.py:1-330](file://backend/app/models/mvp.py#L1-L330)
...
| `/api/mvp/inbox/{item_id}/ignore` | POST | 忽略条目 | item_id |
| `/api/mvp/inbox/{item_id}/clean` | POST | 单条清洗 | item_id |
| `/api/mvp/inbox/batch-clean` | POST | 批量清洗 | ids |
...
| `/api/mvp/materials/batch-build-knowledge` | POST | 批量构建知识 | BatchBuildKnowledgeRequest |
...
| `/api/mvp/materials/{material_id}/toggle-hot` | POST | 切换爆款状态 | material_id |
| `/api/mvp/materials/{material_id}/tags` | POST | 更新素材标签 | UpdateTagsRequest |
...
| `/api/mvp/knowledge/library-stats` | GET | 获取分库统计 | 无 |
| `/api/mvp/knowledge/library/{library_type}` | GET | 按分库列出 | page, size, keyword |
| `/api/mvp/knowledge/libraries` | GET | 获取分库列表 | 无 |
| `/api/mvp/knowledge/chunks/{knowledge_id}` | GET | 获取知识切块 | 无 |
...
| `/api/mvp/raw-contents/auto-pipeline` | POST | 自动入库Pipeline | AutoPipelineRequest |
| `/api/mvp/raw-contents/auto-pipeline/batch` | POST | 批量自动入库 | AutoPipelineBatchRequest |
...
| 接口 | 方法 | 描述 | 参数 |
|------|------|------|------|
| `/api/mvp/compliance/check` | POST | 合规检查 | ComplianceCheckRequest |
| `/api/mvp/compliance/rules` | GET | 规则列表 | rule_type, risk_level, page, size |
| `/api/mvp/compliance/rules` | POST | 创建规则 | ComplianceRuleRequest |
| `/api/mvp/compliance/rules/{rule_id}` | PUT | 更新规则 | ComplianceRuleRequest |
| `/api/mvp/compliance/rules/{rule_id}` | DELETE | 删除规则 | 无 |
| `/api/mvp/compliance/test` | POST | 测试规则 | ComplianceTestRequest |

### 标签管理接口

| 接口 | 方法 | 描述 | 参数 |
|------|------|------|------|
| `/api/mvp/tags` | GET | 获取标签列表 | type |
| `/api/mvp/tags` | POST | 创建标签 | TagCreateRequest |

### 统计仪表盘接口

| 接口 | 方法 | 描述 | 参数 |
|------|------|------|------|
| `/api/mvp/stats/overview` | GET | 获取统计概览 | 无 |
| `/api/mvp/dashboard/stats` | GET | 获取Dashboard统计 | 无 |

### 反馈闭环接口

| 接口 | 方法 | 描述 | 参数 |
|------|------|------|------|
| `/api/mvp/feedback` | POST | 提交反馈 | FeedbackSubmitRequest |
| `/api/mvp/feedback/stats` | GET | 获取反馈统计 | days |
| `/api/mvp/knowledge/quality/rankings` | GET | 获取质量排行榜 | limit, order |
| `/api/mvp/knowledge/quality/suggestions` | GET | 获取学习建议 | 无 |
| `/api/mvp/knowledge/quality/adjust` | POST | 应用权重调整 | 无 |
| `/api/mvp/feedback/tags` | GET | 获取反馈标签 | 无 |

### 模型管理接口

| 接口 | 方法 | 描述 | 参数 |
|------|------|------|------|
| `/api/mvp/models/embedding` | GET | 获取embedding模型 | 无 |
| `/api/mvp/models/llm` | GET | 获取LLM模型 | 无 |
| `/api/mvp/models/embedding/select` | POST | 选择embedding模型 | model_name |
| `/api/mvp/models/ollama/status` | GET | 获取Ollama状态 | 无 |
| `/api/mvp/models/ollama/pull` | POST | 拉取Ollama模型 | model_name |
| `/api/mvp/models/ollama/{model_name}/info` | GET | 获取模型信息 | 无 |
| `/api/mvp/models/ollama/{model_name}/check` | GET | 检查模型状态 | 无 |
| `/api/mvp/models/benchmark` | POST | 模型性能对比 | model_name, test_text |

**章节来源**
- [backend/app/api/endpoints/mvp_inbox_routes.py:17-251](file://backend/app/api/endpoints/mvp_inbox_routes.py#L17-L251)
- [backend/app/api/endpoints/mvp_material_routes.py:21-143](file://backend/app/api/endpoints/mvp_material_routes.py#L21-L143)
- [backend/app/api/endpoints/mvp_knowledge_routes.py:65-454](file://backend/app/api/endpoints/mvp_knowledge_routes.py#L65-L454)
- [backend/app/api/endpoints/mvp_generation_routes.py:13-87](file://backend/app/api/endpoints/mvp_generation_routes.py#L13-L87)
- [backend/app/api/endpoints/mvp_compliance_routes.py:13-122](file://backend/app/api/endpoints/mvp_compliance_routes.py#L13-L122)
- [backend/app/api/endpoints/mvp_tag_routes.py:12-29](file://backend/app/api/endpoints/mvp_tag_routes.py#L12-L29)
- [backend/app/api/endpoints/mvp_stats_routes.py:11-117](file://backend/app/api/endpoints/mvp_stats_routes.py#L11-L117)
- [backend/app/api/endpoints/mvp_feedback_routes.py:12-99](file://backend/app/api/endpoints/mvp_feedback_routes.py#L12-L99)
- [backend/app/api/endpoints/mvp_model_routes.py:12-136](file://backend/app/api/endpoints/mvp_model_routes.py#L12-L136)
...
MVP内容管理系统经过重大重构，从单一mvp_routes.py演进为完整的MVP生态系统。系统通过模块化的设计和分层架构，实现了从内容采集到AI生成的完整闭环。

### 主要优势

1. **完整的功能体系**：涵盖内容管理、AI生成、合规审核等核心功能
2. **模块化架构设计**：20个专门路由模块，职责分离清晰
3. **灵活的架构设计**：支持本地部署和云端部署两种模式
4. **强大的AI集成**：支持多种AI模型和多模态内容处理
5. **完善的监控体系**：提供全面的健康检查和性能监控
6. **良好的扩展性**：模块化设计便于功能扩展和定制
7. **完整的处理管道**：支持从采集到入库的全流程自动化
8. **先进的知识管理**：支持知识图谱和智能关系发现
9. **反馈驱动优化**：基于用户反馈的持续学习和改进
10. **模型即服务**：支持多种AI模型的管理和性能对比
...
该系统为金融行业的内容创作提供了强有力的技术支撑，能够显著提升内容生产的效率和质量。通过模块化的MVP生态系统，系统不仅满足了当前的功能需求，还为未来的功能扩展和技术演进奠定了坚实的基础。