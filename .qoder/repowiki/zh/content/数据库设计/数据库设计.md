...
J["20260323_0008_legacy_baseline.py<br/>基线桥接"]
K["20260323_01_add_leads_and_customer_lead_id.py<br/>线索与客户关联"]
end
subgraph "数据库"
PG["PostgreSQL"]
end
...
**图表来源**
- [models.py:1-1113](file://backend/app/models/models.py#L1-L1113)
- [database.py:1-29](file://backend/app/core/database.py#L1-L29)
- [config.py:27-36](file://backend/app/core/config.py#L27-L36)
- [mvp_inbox_service.py:1-136](file://backend/app/services/mvp_inbox_service.py#L1-L136)
...
## 架构总览
下图展示数据库层与应用层的交互关系，以及迁移工具对模型元数据的依赖，特别突出了MVP架构的完整工作流。
...
**图表来源**
- [models.py:45-1113](file://backend/app/models/models.py#L45-L1113)
...
**章节来源**
- [models.py:949-956](file://backend/app/models/models.py#L949-L956)
- [mvp_inbox_service.py:1-136](file://backend/app/services/mvp_inbox_service.py#L1-L136)
...
**章节来源**
- [models.py:959-981](file://backend/app/models/models.py#L959-L981)
- [models.py:983-1004](file://backend/app/models/models.py#L983-L1004)
...
**章节来源**
- [models.py:1006-1050](file://backend/app/models/models.py#L1006-L1050)
- [models.py:1052-1070](file://backend/app/models/models.py#L1052-L1070)
...
**章节来源**
- [models.py:1084-1102](file://backend/app/models/models.py#L1084-L1102)
...
**章节来源**
- [models.py:1104-1113](file://backend/app/models/models.py#L1104-L1113)
...
**章节来源**
- [models.py:983-995](file://backend/app/models/models.py#L983-L995)
- [models.py:998-1004](file://backend/app/models/models.py#L998-L1004)
...
**章节来源**
- [models.py:1072-1082](file://backend/app/models/models.py#L1072-L1082)
...
- **MVP新增表**：
  - mvp_inbox_items：收件箱条目，包含平台、标题、内容、作者、来源URL、来源类型、关键词、风险等级、重复状态、评分、技术状态、业务状态。
  - mvp_material_items：素材条目，包含平台、标题、内容、来源URL、互动数据、作者、是否爆款、风险等级、使用计数、来源收件箱ID。
  - mvp_tags：标签，包含名称、类型、创建时间。
  - mvp_material_tag_rel：素材-标签关联表。
  - mvp_knowledge_items：知识条目，包含标题、内容、分类、平台、受众、风格、来源素材ID、使用计数、增强字段、分库类型、层级。
  - mvp_knowledge_chunks：知识切块，包含知识ID、切块类型、切块索引、内容、元数据、嵌入向量、token计数。
...
- **MVP示例数据**：
  - 收件箱条目：从社交媒体平台采集的原始内容，包含标题、正文、作者、平台信息。
  - 素材条目：经过筛选和标签化的优质内容，包含互动数据和风险评估。
  - 知识条目：结构化的知识内容，包含主题、受众、风格等增强字段。
...
- **MVP访问模式**：
  - 收件箱：支持分页、筛选、批量处理。
  - 素材库：支持多维度检索、标签管理、使用统计。
  - 知识库：支持关键词检索、向量化检索、分库统计。
