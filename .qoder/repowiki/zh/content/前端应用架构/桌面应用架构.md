...
- [desktop/src/utils/clipboard.ts](file://desktop/src/utils/clipboard.ts)
...
## 更新摘要
**变更内容**
- 新增跨平台剪贴板功能模块，支持HTTP和HTTPS环境下的文本复制
- 在AI工作台、收件箱、素材管理等多个页面集成剪贴板功能
- 实现现代Clipboard API与传统execCommand降级方案的兼容性处理
- 增强用户体验，提供一键复制文本内容的能力
...
- [desktop/src/utils/clipboard.ts:1-37](file://desktop/src/utils/clipboard.ts#L1-L37)
...
- **新增** 剪贴板工具：提供跨平台文本复制功能，支持现代 Clipboard API 与传统降级方案
...
**更新** 新增剪贴板工具模块，提供跨平台文本复制功能，支持现代 Clipboard API 与传统降级方案
...
participant Clipboard as "剪贴板工具 clipboard.ts"
App->>Main : "应用启动"
Main->>Main : "加载/保存配置"
Main->>Main : "启动后端进程"
Main->>Win : "创建窗口并加载资源"
...
Renderer->>Clipboard : "调用 copyToClipboard()"
Clipboard->>Clipboard : "检测安全上下文"
Clipboard-->>Renderer : "返回复制结果"
...
- [desktop/src/utils/clipboard.ts:8-36](file://desktop/src/utils/clipboard.ts#L8-L36)
...
### 剪贴板工具模块
职责与流程要点：
- **新增** 跨平台剪贴板功能：提供统一的文本复制接口，支持 HTTP 和 HTTPS 环境
- **安全上下文检测**：优先使用现代 Clipboard API（仅在安全上下文可用），在 HTTP 环境下降级使用 document.execCommand('copy')
- **错误处理**：捕获复制过程中的异常，提供友好的错误反馈
- **用户体验**：复制成功后提供视觉反馈，提升用户操作体验

**更新** 新增剪贴板工具模块，提供跨平台文本复制功能：

```mermaid
flowchart TD
copyToClipboard["copyToClipboard(text)"] --> CheckSecure{"检查安全上下文"}
CheckSecure --> |HTTPS| ModernAPI["使用 Clipboard API"]
CheckSecure --> |HTTP| LegacyMethod["使用 execCommand('copy')"]
ModernAPI --> Success1["返回 true"]
LegacyMethod --> CreateTextArea["创建隐藏 textarea"]
CreateTextArea --> FocusSelect["focus() + select()"]
FocusSelect --> ExecCopy["execCommand('copy')"]
ExecCopy --> RemoveElement["移除元素"]
RemoveElement --> Success2["返回复制结果"]
Success1 --> End["结束"]
Success2 --> End
```
...
InboxPage["InboxPage 收件箱页面"] --> clipboard["剪贴板工具"]
MaterialsPage["MaterialsPage 素材页面"] --> clipboard
MvpWorkbenchPage["MvpWorkbenchPage 工作台"] --> clipboard
clipboard --> ModernAPI["现代 Clipboard API"]
clipboard --> LegacyMethod["execCommand('copy')"]
...
- [desktop/src/pages/inbox/InboxPage.tsx:590-600](file://desktop/src/pages/inbox/InboxPage.tsx#L590-L600)
- [desktop/src/pages/materials/MaterialsPage.tsx:435-450](file://desktop/src/pages/materials/MaterialsPage.tsx#L435-L450)
- [desktop/src/pages/ai-workbench/MvpWorkbenchPage.tsx:306-330](file://desktop/src/pages/ai-workbench/MvpWorkbenchPage.tsx#L306-L330)
- [desktop/src/utils/clipboard.ts:8-36](file://desktop/src/utils/clipboard.ts#L8-L36)

章节来源
- [desktop/src/pages/inbox/MvpInboxPage.tsx:1-868](file://desktop/src/pages/inbox/MvpInboxPage.tsx#L1-L868)
- [desktop/src/pages/inbox/InboxPage.tsx:1-674](file://desktop/src/pages/inbox/InboxPage.tsx#L1-L674)
- [desktop/src/pages/materials/MaterialsPage.tsx:1-489](file://desktop/src/pages/materials/MaterialsPage.tsx#L1-L489)
...
InboxPage["InboxPage.tsx"] --> clipboard["clipboard.ts"]
MaterialsPage["MaterialsPage.tsx"] --> clipboard
MvpWorkbenchPage["MvpWorkbenchPage.tsx"] --> clipboard
clipboard --> Types["types.ts"]
...
- **新增** 剪贴板功能异常
  - 现象：复制操作无响应或报错
  - 排查：检查浏览器安全上下文、Clipboard API 支持情况、execCommand 降级方案
...
**更新** 新增的剪贴板工具模块显著提升了应用的用户体验，提供了跨平台的文本复制能力，支持现代 Clipboard API 与传统降级方案。同时新增的 inboxApi 和 MvpInboxPage 页面显著增强了应用的功能完整性，提供了完整的管道功能操作能力，包括：
- 全面的收件箱操作 API 覆盖
- 批量处理能力，提升工作效率
- Mock 数据支持，改善用户体验
- 专门的收件箱管理界面，提供直观的操作体验
- 跨平台剪贴板功能，支持 HTTP 和 HTTPS 环境下的文本复制
...
- **新增** 剪贴板 API 使用指南
  - 跨平台兼容：自动检测安全上下文，优先使用现代 Clipboard API
  - HTTP 环境降级：在非安全上下文中使用 execCommand('copy') 方案
  - 错误处理：捕获异常并提供友好的错误反馈
  - 用户体验：复制成功后的视觉反馈机制
- **新增** 收件箱 API 使用指南
  - 列表查询：支持分页与多维度筛选
