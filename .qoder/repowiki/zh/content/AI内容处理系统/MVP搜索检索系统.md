...
- [models.py:937-1113](file://backend/app/models/models.py#L937-L1113)
...
- 数据模型（models.py）：定义MVP核心表结构，包括收件箱、素材库、知识库、知识切块等。
...
Search-->>API : 统一结果结构
API-->>Client : JSON响应
Client->>API : POST /api/mvp/knowledge/search
API->>Knowledge : search_knowledge(query, platform, audience)
Knowledge->>DB : 关键词检索
Knowledge->>DB : 更新use_count
DB-->>Knowledge : 知识结果
Knowledge-->>API : 知识结果
API-->>Client : JSON响应
Client->>API : POST /api/mvp/knowledge/reindex
API->>Knowledge : reindex_knowledge()
Knowledge->>Hybrid : search()v2
Hybrid->>DB : 向量检索
DB-->>Hybrid : 向量结果
Hybrid-->>Knowledge : 重排序结果
Knowledge-->>API : 重建索引结果
API-->>Client : JSON响应
```

**图表来源**
- [mvp_routes.py:414-430](file://backend/app/api/endpoints/mvp_routes.py#L414-L430)
- [mvp_routes.py:336-380](file://backend/app/api/endpoints/mvp_routes.py#L336-L380)
- [mvp_search_service.py:75-131](file://backend/app/services/mvp_search_service.py#L75-L131)
- [mvp_knowledge_service.py:101-124](file://backend/app/services/mvp_knowledge_service.py#L101-L124)
...
## 依赖关系分析
- API路由依赖服务层：统一检索、知识检索、重建索引等端点均委托给对应服务。
...
## 结论
MVP搜索检索系统通过关键词与向量双路径融合，结合元数据过滤与重排序机制，提供了高效、可扩展的知识检索能力。系统以清晰的分层架构组织代码，配合完善的模型与服务，能够满足内容创作与合规审核场景下的多源检索需求，并为后续的向量化能力演进奠定了坚实基础。